...
The `GroupDocs.Total.Mcp` package is an **MCP server** (the csproj declares
...
[nuget.org/packages/GroupDocs.Total.Mcp](https://www.nuget.org/packages/GroupDocs.Total.Mcp)
...
```bash
curl -L -o pkg.nupkg \
  https://www.nuget.org/api/v2/package/GroupDocs.Total.Mcp/26.7.0
...
```json
{
  ...
  "name": "io.github.groupdocs-total/groupdocs-total-mcp",
  "version": "26.7.0",
  "packages": [ { "registryType": "nuget", "identifier": "GroupDocs.Total.Mcp", ... } ],
  "repository": { "url": "https://github.com/groupdocs-total/GroupDocs.Total.Mcp" }
}
```
...
3. Search for `groupdocs-total` or `total`.
4. The package should appear with the description from `server.json`:
   *"MCP server for GroupDocs.Total — add, list, update, remove document annotations via AI agents."*
...
- `dnx` successfully resolves `GroupDocs.Total.Mcp@26.7.0` from nuget.org.
...
curl -s https://registry.modelcontextprotocol.io/v0/servers \
  | jq '.[] | select(.name | contains("groupdocs-total"))'
```
...
- `ToolDiscoveryTests.ServerInfo_AdvertisesGroupDocsTotalMcp` — the server
  reports `serverInfo.name == "GroupDocs.Total.Mcp"` and a non-empty version
