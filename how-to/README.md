...
[`GroupDocs.Total.Mcp`](https://www.nuget.org/packages/GroupDocs.Total.Mcp).
...
- All guides target `GroupDocs.Total.Mcp@26.7.0`. Substitute a newer version
  freely — the interfaces haven't changed.
- The server exposes **38 tools across 10 product domains** (Annotation,
  Comparison, Conversion, Markdown, Merger, Metadata, Parser, Redaction,
  Signature, Watermark) plus cross-product `GetDocumentInfo` and
  `GetDocumentPageImage`. Tool names use the `{Product}{Verb}{Noun}` PascalCase
  convention; wire names are their snake_case forms.
- Tools that call `Save()` (Conversion, Watermark, Merger, Redaction, Signature
  sign, …) require a licensed `GroupDocs.Total.lic` — without one, output is
  watermarked or blocked. See each guide's "License" section.
