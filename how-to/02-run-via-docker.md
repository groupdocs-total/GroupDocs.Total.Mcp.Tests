...
- `ghcr.io/groupdocs-total/total-net-mcp` — GitHub Container Registry (primary)
- `docker.io/groupdocs/total-net-mcp` — Docker Hub (mirror)
...
```bash
docker pull ghcr.io/groupdocs-total/total-net-mcp:26.5.0   # pinned to exact version
docker pull ghcr.io/groupdocs-total/total-net-mcp:latest   # floats to most recent push
```
...
docker pull ghcr.io/groupdocs-total/total-net-mcp:latest
...
```bash
docker inspect ghcr.io/groupdocs-total/total-net-mcp:latest \
...
(
  ...
  sleep 2
    ...
    -v $(pwd)/documents:/data \
    ghcr.io/groupdocs-total/total-net-mcp:26.5.0 2>/dev/null
```
...
services:
  groupdocs-total-mcp:
    image: ghcr.io/groupdocs-total/total-net-mcp:26.5.0
    stdin_open: true
    tty: false
    environment:
...
docker inspect ghcr.io/groupdocs-total/total-net-mcp:26.5.0 \
...
| `manifest unknown` / can't pull image | Version tag doesn't exist on that registry | Check [ghcr.io/groupdocs-total/total-net-mcp](https://github.com/groupdocs-total/GroupDocs.Total.Mcp/pkgs/container/total-net-mcp) for available tags. |
| `:latest` ran but didn't pick up a new release | Docker reuses cached image — `docker run :latest` does not auto-pull | `docker pull ghcr.io/groupdocs-total/total-net-mcp:latest` before running, or add `--pull always` to your `docker run`. |
