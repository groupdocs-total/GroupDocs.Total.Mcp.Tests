...
public class ComparisonCompareToolTests
{
    ...
    public ComparisonCompareToolTests(McpServerFixture fixture, ITestOutputHelper output)
    {
        _fixture = fixture;
        _output = output;
    }
    ...
    [Fact]
    public async Task ComparisonCompare_SmokeCall_ReturnsResponseAndDoesNotCrashServer()
    {
        var catalog = await ToolCatalog.LoadAsync(_fixture.Client);
        var tool = catalog.ByName("ComparisonCompare");
        ...
        try
        {
            var response = await _fixture.Client.CallToolAsync(
                ...
                ["targetFile"] = new Dictionary<string, object?> { ["filePath"] = SampleDocuments.BlankPdf },
                });
