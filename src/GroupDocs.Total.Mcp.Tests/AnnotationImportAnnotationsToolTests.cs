...
public class AnnotationImportAnnotationsToolTests
{
    ...
    public AnnotationImportAnnotationsToolTests(McpServerFixture fixture, ITestOutputHelper output)
    {
        _fixture = fixture;
        _output = output;
    }
    ...
    [Fact]
    public async Task AnnotationImportAnnotations_SmokeCall_ReturnsResponseAndDoesNotCrashServer()
    {
        var catalog = await ToolCatalog.LoadAsync(_fixture.Client);
        var tool = catalog.ByName("AnnotationImportAnnotations");
        ...
        try
        {
            var response = await _fixture.Client.CallToolAsync(
                ...
                ["source"] = new Dictionary<string, object?> { ["filePath"] = "missing.xml" },
                });
