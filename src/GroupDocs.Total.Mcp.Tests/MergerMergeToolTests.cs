...
public class MergerMergeToolTests
{
    ...
    public MergerMergeToolTests(McpServerFixture fixture, ITestOutputHelper output)
    {
        _fixture = fixture;
        _output = output;
    }
    ...
    [Fact]
    public async Task MergerMerge_SmokeCall_ReturnsResponseAndDoesNotCrashServer()
    {
        var catalog = await ToolCatalog.LoadAsync(_fixture.Client);
        var tool = catalog.ByName("MergerMerge");
        ...
        try
        {
            var response = await _fixture.Client.CallToolAsync(
                ...
                ["file2"] = new Dictionary<string, object?> { ["filePath"] = SampleDocuments.BlankPdf },
                });
